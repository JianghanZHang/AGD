#ifndef MIM_SOLVERS_GRG_ADAH_HPP_
#define MIM_SOLVERS_GRG_ADAH_HPP_
...
class SolverGRG_ADAHESSIAN : public crocoddyl::SolverAbstract {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  
  typedef typename crocoddyl::MathBaseTpl<double>::MatrixXsRowMajor MatrixXdRowMajor;

  explicit SolverGRG_ADAHESSIAN(boost::shared_ptr<crocoddyl::ShootingProblem> problem);
  virtual ~SolverGRG_ADAHESSIAN();
  ...
  double get_const_step_length() const { return const_step_length_; };
  std::vector<double> get_alphas() const { return alphas_; };
  bool get_use_line_search() const { return use_filter_line_search_; };
  ...
  void set_use_filter_line_search(bool use_filter_line_search) { use_filter_line_search_ = use_filter_line_search; };
  void set_use_line_search(bool use_line_search) { use_line_search = use_line_search; };
  ...
  void set_const_step_length (double const_step_length) { const_step_length_ = const_step_length; };
 ...
 public:
  ...
  std::vector<Eigen::VectorXd> D_square_;                     //!< the squared diagonal of the Hessian matrix
  std::vector<Eigen::VectorXd> m_;                     //!< the squared Qu matrix
  std::vector<Eigen::VectorXd> v_;                     //!< the squared Qu matrix
  std::vector<Eigen::VectorXd> D_;                    //!< the diagonal of the Hessian matrix
  std::vector<Eigen::VectorXd> m_corrected_;                     //!< the squared Qu matrix
  std::vector<Eigen::VectorXd> v_corrected_;                     //!< the squared Qu matrix

  std::vector<Eigen::MatrixXd> Hxx_;                     //!< the hessian matrix Hxx
  std::vector<Eigen::MatrixXd> Huu_;                     //!< the hessian matrix Huu

  std::vector<Eigen::MatrixXd> FuTHxx_p_;                     //!< tmp variable
  Eigen::MatrixXd FxTHxx_p_;                     //!< tmp variable
...
#endif  // MIM_SOLVERS_GRG_ADAH_HPP_
