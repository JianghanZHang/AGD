...
int main(){
    ...
    // const int nthreads = int(std::cin.get());

    problem->set_nthreads(8);
    problem->set_x0(x0);
    ...
    // Unonstrained case
    // DEFINE SOLVER
    crocoddyl::SolverDDP solver_DDP = crocoddyl::SolverDDP(problem);

    grg::SolverGRG_ADAHESSIAN solver_GRGADAH = grg::SolverGRG_ADAHESSIAN(problem);
    ...
    const int max_iter_GRG = 100;

    // SETTING UP STATISTICS
    const int nb_DDP = 2;
    const int nb_GRG = 2;
    const int nb_GRGADAH = 2;

    Eigen::VectorXd duration_DDP(nb_DDP);
    Eigen::VectorXd duration_GRG(nb_GRG);
    Eigen::VectorXd duration_GRGADAH(nb_GRGADAH);
    ...
    for (unsigned i = 0; i < nb_GRG; ++i){
     ...
     for (unsigned i = 0; i < nb_GRG; ++i){
        timer.start();
        solver_GRGADAH.solve(xs, us, max_iter_GRG);
        timer.stop();
        duration_GRGADAH[i] = timer.elapsed().user / max_iter_GRG;
    }
    ...
    double const std_dev_GRGADAH = std::sqrt((duration_GRGADAH.array() - duration_GRGADAH.mean()).square().sum() / (nb_GRG - 1));
    ...
    std::cout << "  " << std::left << std::setw(42) << "UR5 GRG ADAH" << std::left
              << std::setw(15) << duration_GRGADAH.mean() << std::left << std::setw(15)
              << std_dev_GRGADAH << std::left << std::setw(15)
              << duration_GRGADAH.maxCoeff() << std::left << std::setw(15)
              << duration_GRGADAH.minCoeff() << std::endl;
