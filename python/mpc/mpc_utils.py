########################################################################################
## This code is from https://github.com/machines-in-motion/minimal_examples_crocoddyl ##
########################################################################################
...
def get_contact_wrench(pybullet_simulator, id_endeff):
    ...
    for ci in reversed(contact_points):
        ...
        R_ct = np.vstack([np.array(contact_normal), np.array(lateral_friction_direction_1),
                          np.array(lateral_friction_direction_2)]).T
...
def init_sim_data(sim_params, ocp_params, x0):
    ...
    sim_data['N_mpc'] = int(
        sim_data['T_sim'] * sim_data['mpc_freq'])  # Total number of planning steps in the simulation
    ...
    sim_data['state_pred'] = np.zeros(
        (sim_data['N_mpc'], ocp_params['N_h'] + 1, sim_data['nx']))  # Predicted states  ( ddp.xs : {x* = (q*, v*)} )
    sim_data['ctrl_pred'] = np.zeros(
        (sim_data['N_mpc'], ocp_params['N_h'], sim_data['nu']))  # Predicted torques ( ddp.us : {u*} )
    sim_data['force_pred'] = np.zeros((sim_data['N_mpc'], ocp_params['N_h'], 6))  #  Predicted EE contact forces
    sim_data['state_des_MPC_RATE'] = np.zeros((sim_data['N_mpc'] + 1, sim_data[
        'nx']))  # Predicted states at planner frequency  ( x* interpolated at PLAN freq )
    sim_data['ctrl_des_MPC_RATE'] = np.zeros(
        (sim_data['N_mpc'], sim_data['nu']))  # Predicted torques at planner frequency ( u* interpolated at PLAN freq )
    ...
    sim_data['state_des_SIM_RATE'] = np.zeros(
        (sim_data['N_sim'] + 1, sim_data['nx']))  # Reference state at actuation freq ( x* interpolated at SIMU freq )
    sim_data['ctrl_des_SIM_RATE'] = np.zeros(
        (sim_data['N_sim'], sim_data['nu']))  # Reference input at actuation freq ( u* interpolated at SIMU freq )
    ...
    sim_data['state_mea_SIM_RATE'] = np.zeros(
        (sim_data['N_sim'] + 1, sim_data['nx']))  # Measured states ( x^mea = (q, v) from actuator & PyB at SIMU freq )
    ...
    print('- Simulation frequency                 : f_simu          = ' + str(
        float(sim_data['sim_freq'] / 1000.)) + ' kHz')
    print('- Replanning frequency                 : f_plan          = ' + str(
        float(sim_data['mpc_freq'] / 1000.)) + ' kHz')
...
def extract_plot_data_from_sim_data(sim_data):
    ...
    for node_id in range(sim_data['N_h'] + 1):
        plot_data['lin_pos_ee_pred'][:, node_id, :] = pin_utils.get_p_(plot_data['q_pred'][:, node_id, :],
                                                                       plot_data['pin_model'], sim_data['id_endeff'])
        plot_data['lin_vel_ee_pred'][:, node_id, :] = pin_utils.get_v_(plot_data['q_pred'][:, node_id, :],
                                                                       plot_data['v_pred'][:, node_id, :],
                                                                       plot_data['pin_model'], sim_data['id_endeff'])
        plot_data['ang_pos_ee_pred'][:, node_id, :] = pin_utils.get_rpy_(plot_data['q_pred'][:, node_id, :],
                                                                         plot_data['pin_model'], sim_data['id_endeff'])
        plot_data['ang_vel_ee_pred'][:, node_id, :] = pin_utils.get_w_(plot_data['q_pred'][:, node_id, :],
                                                                       plot_data['v_pred'][:, node_id, :],
                                                                       plot_data['pin_model'], sim_data['id_endeff'])
    ...
    plot_data['lin_vel_ee_mea'] = pin_utils.get_v_(plot_data['q_mea'], plot_data['v_mea'], sim_data['pin_model'],
                                                   sim_data['id_endeff'])
    # Angular
    plot_data['ang_pos_ee_mea'] = pin_utils.get_rpy_(plot_data['q_mea'], sim_data['pin_model'], sim_data['id_endeff'])
    plot_data['ang_vel_ee_mea'] = pin_utils.get_w_(plot_data['q_mea'], plot_data['v_mea'], sim_data['pin_model'],
                                                   sim_data['id_endeff'])
    # EE des
    # Linear
    plot_data['lin_pos_ee_des_MPC_RATE'] = pin_utils.get_p_(plot_data['q_des_MPC_RATE'], sim_data['pin_model'],
                                                            sim_data['id_endeff'])
    plot_data['lin_vel_ee_des_MPC_RATE'] = pin_utils.get_v_(plot_data['q_des_MPC_RATE'], plot_data['v_des_MPC_RATE'],
                                                            sim_data['pin_model'], sim_data['id_endeff'])
    plot_data['lin_pos_ee_des_SIM_RATE'] = pin_utils.get_p_(plot_data['q_des_SIM_RATE'], sim_data['pin_model'],
                                                            sim_data['id_endeff'])
    plot_data['lin_vel_ee_des_SIM_RATE'] = pin_utils.get_v_(plot_data['q_des_SIM_RATE'], plot_data['v_des_SIM_RATE'],
                                                            sim_data['pin_model'], sim_data['id_endeff'])
    # Angular
    plot_data['ang_pos_ee_des_MPC_RATE'] = pin_utils.get_rpy_(plot_data['q_des_MPC_RATE'], sim_data['pin_model'],
                                                              sim_data['id_endeff'])
    plot_data['ang_vel_ee_des_MPC_RATE'] = pin_utils.get_w_(plot_data['q_des_MPC_RATE'], plot_data['v_des_MPC_RATE'],
                                                            sim_data['pin_model'], sim_data['id_endeff'])
    plot_data['ang_pos_ee_des_SIM_RATE'] = pin_utils.get_rpy_(plot_data['q_des_SIM_RATE'], sim_data['pin_model'],
                                                              sim_data['id_endeff'])
    plot_data['ang_vel_ee_des_SIM_RATE'] = pin_utils.get_w_(plot_data['q_des_SIM_RATE'], plot_data['v_des_SIM_RATE'],
                                                            sim_data['pin_model'], sim_data['id_endeff'])
    #  Extract EE force
    ...
    for i in range(nq):
        ...
        if (PLOT_PREDICTIONS):
            ...
            for j in range(0, N_mpc, pred_plot_sampling):
                ...
                my_colors = cm(colors)
                ax_x[i, 0].scatter(tspan_x_pred, q_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                   cmap=matplotlib.cm.Greys)  # c='black',
                ax_x[i, 1].scatter(tspan_x_pred, v_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                   cmap=matplotlib.cm.Greys)  # c='black',

        #  Joint position
        ax_x[i, 0].plot(t_span_plan, plot_data['q_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                        label='Predicted', alpha=0.1)
        ...
        if ('stateReg' in plot_data['active_costs']):
            ax_x[i, 0].plot(t_span_plan[:-1], plot_data['state_ref'][:, i], linestyle='-.', color='k', marker=None,
                            label='xReg_ref', alpha=0.5)
        ...
        #  Joint velocity
        ax_x[i, 1].plot(t_span_plan, plot_data['v_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
        ...
        if ('stateReg' in plot_data['active_costs']):
            ax_x[i, 1].plot(t_span_plan[:-1], plot_data['state_ref'][:, i + nq], linestyle='-.', color='k', marker=None,
                            label='xReg_ref', alpha=0.5)
    ...
    for i in range(nq):
        ...
        if (PLOT_PREDICTIONS):
            ...
            for j in range(0, N_mpc, pred_plot_sampling):
                ...
                colors = np.r_[np.linspace(0.1, 1, N_h), 1]
                my_colors = cm(colors)
                ax_u[i].scatter(tspan_u_pred, u_pred_i[j, :], s=10, zorder=1,
                                c=cm(np.r_[np.linspace(0.1, 1, N_h - 1), 1]), cmap=matplotlib.cm.Greys)  # c='black'

        #  Joint torques
        ax_u[i].plot(t_span_plan, plot_data['u_pred'][:, 0, i], color='r', marker=None, linestyle='-',
                     label='Optimal control u0*', alpha=0.6)
        ax_u[i].plot(t_span_plan, plot_data['u_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                     label='Predicted', alpha=0.1)
        ...
        if ('ctrlReg' or 'ctrlRegGrav' in plot_data['active_costs']):
            ax_u[i].plot(t_span_plan, plot_data['ctrl_ref'][:, i], linestyle='-.', color='k', marker=None,
                         label='uReg_ref', alpha=0.5)
    ...
    for i in range(3):
        ...
        if (PLOT_PREDICTIONS):
            ...
            for j in range(0, N_mpc, pred_plot_sampling):
                ...
                colors = np.r_[np.linspace(0.1, 1, N_h), 1]
                my_colors = cm(colors)
                ax[i, 0].scatter(tspan_x_pred, lin_pos_ee_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                 cmap=matplotlib.cm.Greys)
                ax[i, 1].scatter(tspan_x_pred, lin_vel_ee_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                 cmap=matplotlib.cm.Greys)

        #  EE position
        ax[i, 0].plot(t_span_plan, plot_data['lin_pos_ee_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        # ax[i,0].plot(t_span_simu, plot_data['lin_pos_ee_des_SIM_RATE'][:,i], color='y', linestyle='-', marker='.', label='Predicted (SIMU rate)', alpha=0.5)
        ax[i, 0].plot(t_span_simu, plot_data['lin_pos_ee_mea'][:, i], 'r-', label='Measured (WITH noise)', linewidth=1,
                      alpha=0.3)
        #  Plot reference
        if ('translation' in plot_data['active_costs']):
            ax[i, 0].plot(t_span_plan[:-1], plot_data['lin_pos_ee_ref'][:, i], color='k', linestyle='-.', linewidth=2.,
                          label='Reference', alpha=0.9)
        ...
        #  EE velocity
        ax[i, 1].plot(t_span_plan, plot_data['lin_vel_ee_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        # ax[i,1].plot(t_span_simu, plot_data['lin_vel_ee_des_SIM_RATE'][:,i], color='y', linestyle='-', marker='.', label='Predicted (SIMU rate)', alpha=0.5)
        ax[i, 1].plot(t_span_simu, plot_data['lin_vel_ee_mea'][:, i], 'r-', label='Measured (WITH noise)', linewidth=1,
                      alpha=0.3)
        #  Plot reference
        if ('velocity' in plot_data['active_costs']):
            ax[i, 1].plot(t_span_plan, [0.] * (N_mpc + 1), color='k', linestyle='-.', linewidth=2., label='Reference',
                          alpha=0.9)
    ...
    for i in range(3):
        ...
        if (PLOT_PREDICTIONS):
            ...
            for j in range(0, N_mpc, pred_plot_sampling):
                ...
                colors = np.r_[np.linspace(0.1, 1, N_h), 1]
                my_colors = cm(colors)
                ax[i, 0].scatter(tspan_x_pred, ang_pos_ee_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                 cmap=matplotlib.cm.Greys)
                ax[i, 1].scatter(tspan_x_pred, ang_vel_ee_pred_i[j, :], s=10, zorder=1, c=my_colors,
                                 cmap=matplotlib.cm.Greys)

        #  EE position
        ax[i, 0].plot(t_span_plan, plot_data['ang_pos_ee_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        # ax[i,0].plot(t_span_simu, plot_data['ang_pos_ee_des_SIM_RATE'][:,i], color='y', linestyle='-', marker='.', label='Predicted (SIMU rate)', alpha=0.5)
        ax[i, 0].plot(t_span_simu, plot_data['ang_pos_ee_mea'][:, i], 'r-', label='Measured (WITH noise)', linewidth=1,
                      alpha=0.3)
        #  Plot reference
        if ('rotation' in plot_data['active_costs']):
            ax[i, 0].plot(t_span_plan[:-1], plot_data['ang_pos_ee_ref'][:, i], 'm-.', linewidth=2., label='Reference',
                          alpha=0.9)
        ...
        #  EE velocity
        ax[i, 1].plot(t_span_plan, plot_data['ang_vel_ee_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        # ax[i,1].plot(t_span_simu, plot_data['ang_vel_ee_des_SIM_RATE'][:,i], color='y', linestyle='-', marker='.', label='Predicted (SIMU rate)', alpha=0.5)
        ax[i, 1].plot(t_span_simu, plot_data['ang_vel_ee_mea'][:, i], 'r-', label='Measured (WITH noise)', linewidth=1,
                      alpha=0.3)
        #  Plot reference
        if ('velocity' in plot_data['active_costs']):
...
def plot_mpc_force(plot_data, PLOT_PREDICTIONS=False,
    ...
    for i in range(3):
        ...
        #  EE linear force
        ax[i, 0].plot(t_span_plan, plot_data['f_ee_des_MPC_RATE'][:, i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        ...
        if ('force' in plot_data['active_costs']):
            ax[i, 0].plot(t_span_plan, plot_data['f_ee_ref'][:, i], color=[0., 1., 0., 0.], linestyle='-.',
                          linewidth=2., label='Reference', alpha=0.9)
        ...
        #  EE angular force
        ax[i, 1].plot(t_span_plan, plot_data['f_ee_des_MPC_RATE'][:, 3 + i], color='b', linestyle='-', marker='.',
                      label='Predicted ', alpha=0.1)
        ...
        #  Plot reference
        if ('force' in plot_data['active_costs']):
            ax[i, 1].plot(t_span_plan, plot_data['f_ee_ref'][:, 3 + i], color=[0., 1., 0., 0.], linestyle='-.',
                          linewidth=2., label='Reference', alpha=0.9)
